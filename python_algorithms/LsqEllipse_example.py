...
def get_args():
    ...
    parser.add_argument('--center', nargs=2, type=float, default=(30, 17), metavar=('X', 'Y'))
    parser.add_argument('--width', default=1, type=float)
    parser.add_argument('--height', default=.6, type=float)
    parser.add_argument('--phi', default=np.pi/3, type=float)
    parser.add_argument('--noise', default=.05, type=float)
...
def make_test_ellipse(center, width, height, phi, noise):
    ...
    df = pd.DataFrame({'x':ellipse_x,'y':ellipse_y});
    df.to_csv('ellipse_test.csv',index=False)
